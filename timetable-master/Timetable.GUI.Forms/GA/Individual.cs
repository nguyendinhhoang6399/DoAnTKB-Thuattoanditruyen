...
namespace Timetable.GUI.Forms.GA
{
	 ...
	 */
        ...
        public Individual(Timetable timetable)
        {
            ...
            // 1 gene for room, 1 for time, 1 for professor, 1 for "số tiết mỗi buổi"
            int chromosomeLength = numClasses * 4;
            // Create random individual
            int[] newChromosome = new int[chromosomeLength];
            int chromosomeIndex = 0;
            // Loop through modules
            foreach (HocPhanData hocphan in timetable.Modules.ListHocPhan)
            {
                //Bỏ qua học phần không sắp thời khóa biểu
                if (hocphan.IsScheduled != 0)
                    continue;
                // Loop through groups
                foreach (NhomHPData nhomHP in timetable.getGroupModules(hocphan.MaHocPhan.Trim()))
                {
                    int professorID = timetable.getRandomProfessor(hocphan.MaHocPhan.Trim());

                    //Lặp số buổi học trong group
                    for (int i = 0; i < nhomHP.TongBuoiHoc; i++)
                    {
                        // Add random time
                        TimeslotData time = timetable.getRandomTimeslot();
                        int timeslotId = time.TimeslotID;
                        newChromosome[chromosomeIndex] = timeslotId;
                        chromosomeIndex++;

                        // Add random room
                        int roomId = timetable.getRandomRoom().IdPhongHoc;
                        ...
                        //Add số tiết mỗi buổi
                        int soTiet = (int)(nhomHP.TongTietHoc / nhomHP.TongBuoiHoc);
                        newChromosome[chromosomeIndex] = soTiet;
                        chromosomeIndex++;
                    }
                }
            }
